...
export default function Home() {
  ...
  const saveReading = async (e: React.FormEvent) => {
    ...
    if (!formShipment) { setError("Cadastre a etiqueta antes de registrar uma leitura."); return; }
    if (pending.some(item => item.tagId === formShipment.tagId && item.stage === "Recebimento")) { setError("Já existe um recebimento desta carga aguardando sincronização."); return; }
    if (form.status === "normal" && pending.some(item => item.tagId === formShipment.tagId && item.status === "alert")) { setError("Há um alerta aguardando sincronização; o indicador não pode voltar ao estado normal."); return; }
    if (!photo || !sampledColor) { setError("Tire uma foto e toque na área colorida do indicador."); return; }
    if (colorResult !== form.status && !justification.trim()) { setError("A cor está inconclusiva ou difere da condição escolhida. Informe uma justificativa."); return; }
    ...
    try {
      if (navigator.onLine) {
        try { await post("/api/checkins", entry); await refresh(); setNotice("Check-in e foto registrados no servidor."); }
        catch (problem) { if (!(problem instanceof TypeError) && ![502, 503, 504].includes((problem as {status?:number}).status || 0)) throw problem; await queueCheckin(entry); setPending(await pendingCheckins()); setNotice("Sem conexão com o servidor. Registro e foto salvos neste aparelho para sincronizar."); }
      } else { await queueCheckin(entry); setPending(await pendingCheckins()); setNotice("Registro e foto salvos neste aparelho. Serão enviados quando houver conexão."); }
      setPhoto(""); setSampledColor(""); setJustification(""); setLocation(null);
      setSelectedId(formShipment.id); setTab("cargas");
      ...
      <div className="content">
        ...
        <Tabs value={tab} onValueChange={setTab}><TabsList className="mobile-tabs"><TabsTrigger value="painel">Painel</TabsTrigger><TabsTrigger value="leitura">Leitura</TabsTrigger><TabsTrigger value="cargas">Cargas</TabsTrigger></TabsList>
          ...
          <TabsContent value="leitura"><div className="page-heading"><div><p className="eyebrow">CHECK-IN DE CARGA</p><h1>Registrar leitura</h1><p>Identifique a carga, fotografe o indicador e registre a condição na entrega.</p></div></div><div className="form-layout"><form className="panel reading-form" onSubmit={saveReading}><div className="form-section-heading"><span className="step">01</span><div><h2>Identificação</h2><p>Leia a etiqueta NFC ou digite o código impresso.</p></div></div>{webNfcAvailable ? <button type="button" className="scan-button" onClick={() => void scanNfc()}><ScanLine size={27}/><span><strong>Escanear etiqueta NFC</strong><small>Aproxime o celular da tag</small></span><ArrowRight size={19}/></button> : <p className="helper-note">No iPhone, grave a URL da carga na tag NFC, acenda a tela e aproxime a parte superior do aparelho. Toque na notificação para abrir esta ficha. O Safari não ativa um leitor NFC dentro da página.</p>}{scanStatus && <p className="helper-note" role="status">{scanStatus}</p>}
            ...
            <div className="form-divider"/><div className="form-section-heading"><span className="step">02</span><div><h2>Condição no ponto de passagem</h2><p>Registre a etapa, a escola ou ponto de passagem e o responsável.</p></div></div><div className="form-row"><label className="field">Etapa<select value={form.stage} onChange={e => setForm({ ...form, stage: e.target.value })}><option>Expedição</option><option>Checkpoint</option><option>Recebimento</option></select></label><label className="field">Localidade<input required list="place-options" value={form.place} onChange={e => setForm({ ...form, place: e.target.value })}/><datalist id="place-options">{places.map(p => <option value={p} key={p}/>)}</datalist></label></div><label className="field">Responsável pela leitura<input required maxLength={100} placeholder="Nome de quem conferiu a carga" value={form.actor} onChange={e => setForm({ ...form, actor: e.target.value })}/></label>
            <div className="form-divider"/><div className="form-section-heading"><span className="step">03</span><div><h2>Evidência fotográfica</h2><p>Fotografe o indicador, depois toque na região colorida da imagem.</p></div></div><PhotoEvidence photo={photo} sample={sampledColor} onChange={(nextPhoto, sample) => { setPhoto(nextPhoto); setSampledColor(sample); if (sample && formShipment?.intactColor && formShipment.activatedColor) { const result = classifyColor(sample, formShipment.intactColor, formShipment.activatedColor); if (result !== "uncertain") setForm(current => ({ ...current, status: result })); } }}/><p className="helper-note">{sampledColor ? colorResult === "uncertain" ? "Análise inconclusiva: fotografe com outra luz ou justifique sua avaliação." : `Análise da cor: indicador ${colorResult === "normal" ? "íntegro" : "ativado"}. Confirme a condição abaixo.` : "A análise será feita após selecionar a cor na foto."}</p><p className="field-caption">Condição registrada</p><div className="condition-choices"><label className={`condition ${form.status === "normal" ? "picked" : ""}`}><input type="radio" name="condition" checked={form.status === "normal"} onChange={() => setForm({ ...form, status: "normal" })}/><span className="choice-symbol good"><Check size={18}/></span><span><strong>Indicador íntegro</strong><small>Cor original, sem ativação</small></span></label><label className={`condition danger ${form.status === "alert" ? "picked" : ""}`}><input type="radio" name="condition" checked={form.status === "alert"} onChange={() => setForm({ ...form, status: "alert" })}/><span className="choice-symbol hot"><CircleAlert size={18}/></span><span><strong>Indicador ativado</strong><small>Mudança irreversível de cor</small></span></label></div><label className="field">Justificativa (obrigatória se a análise for inconclusiva ou diferente do registro)<textarea maxLength={300} value={justification} onChange={e => setJustification(e.target.value)} placeholder="Explique a inspeção visual e eventual divergência" /></label>
            <div className="geo-row"><div><MapPin size={18}/><span>{geoMessage}</span></div><button type="button" onClick={getLocation}>Capturar GPS</button></div><button className="button-main submit-button" disabled={busy} type="submit">{busy ? "Salvando..." : online ? "Confirmar check-in" : "Guardar check-in offline"}<ArrowRight size={18}/></button></form><aside className="reading-aside"><div className="panel explainer"><div className="aside-icon"><Smartphone size={24}/></div><h3>Evidência para fiscalização</h3><p>A foto e a amostra de cor são anexadas à carga. Sem conexão, o horário do aparelho é registrado e o servidor anota quando recebeu os dados.</p><div className="aside-points"><span><Check size={15}/> NFC identifica a carga</span><span><Check size={15}/> Foto e cor analisadas</span><span><Check size={15}/> Sincronização de registros pendentes</span></div></div><div className="caution-box"><CircleAlert size={18}/><p>A tag NFC não é um termômetro. A análise de cor exige referências calibradas para o indicador físico e pode ser inconclusiva por luz, reflexos ou posição da foto.</p></div></aside></div></TabsContent>
...
function NewLoadDialog({ open, setOpen, value, setValue, onSubmit, busy }: { open: boolean; setOpen: (o:boolean)=>void; value: {tagId:string;product:string;origin:string;destination:string;threshold:string;intactColor:string;activatedColor:string;supplier:string;contractReference:string}; setValue: (v: {tagId:string;product:string;origin:string;destination:string;threshold:string;intactColor:string;activatedColor:string;supplier:string;contractReference:string})=>void; onSubmit: (e:React.FormEvent)=>void; busy:boolean }) { const [confirmed, setConfirmed] = useState(false); return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><button className="button-outline"><Plus size={17}/> Nova carga</button></DialogTrigger><DialogContent className="new-dialog"><DialogHeader><DialogTitle>Cadastrar carga</DialogTitle></DialogHeader><p className="dialog-subtitle">Associe a etiqueta NFC ao lote destinado à alimentação escolar.</p><form onSubmit={onSubmit} className="dialog-form"><label className="field">Código da etiqueta<input required minLength={3} maxLength={50} pattern="[A-Za-z0-9-]+" placeholder="Ex.: TT-SE-001" value={value.tagId} onChange={e=>setValue({...value,tagId:e.target.value.toUpperCase()})}/></label><label className="field">Produto ou lote<input required maxLength={100} placeholder="Ex.: Laticínios, lote 204" value={value.product} onChange={e=>setValue({...value,product:e.target.value})}/></label><div className="form-row"><label className="field">Origem<input required maxLength={100} list="cities-origin" value={value.origin} onChange={e=>setValue({...value,origin:e.target.value})}/></label><label className="field">Escola ou destino<input required maxLength={100} list="cities-origin" value={value.destination} onChange={e=>setValue({...value,destination:e.target.value})}/></label></div><div className="form-row"><label className="field">Fornecedor ou agricultor familiar<input maxLength={100} placeholder="Ex.: Cooperativa local" value={value.supplier} onChange={e=>setValue({...value,supplier:e.target.value})}/></label><label className="field">Contrato ou pedido<input maxLength={100} placeholder="Ex.: Pedido 2026/42" value={value.contractReference} onChange={e=>setValue({...value,contractReference:e.target.value})}/></label></div><datalist id="cities-origin">{places.map(p=><option key={p} value={p}/>)}</datalist><label className="field">Limiar do indicador (°C)<input type="number" min="-50" max="100" step="0.1" required value={value.threshold} onChange={e=>setValue({...value,threshold:e.target.value})}/></label><p className="dialog-help">O limiar descreve o indicador físico, não uma medição pelo celular. Configure as duas cores conforme a embalagem do indicador que será testado.</p><div className="form-row"><label className="field">Cor íntegra<input type="color" value={value.intactColor} onChange={e=>setValue({...value,intactColor:e.target.value})}/></label><label className="field">Cor ativada<input type="color" value={value.activatedColor} onChange={e=>setValue({...value,activatedColor:e.target.value})}/></label></div><p className="dialog-help">Os exemplos de cores precisam ser ajustados ao indicador adquirido e validados com fotos reais. Uma diferença pequena entre as cores reduz a confiabilidade.</p><label className="dialog-help"><input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)}/> Conferi as cores com o indicador físico deste lote.</label><Button type="submit" className="dialog-submit" disabled={busy || !confirmed}>{busy ? "Cadastrando..." : "Cadastrar e iniciar"}</Button></form></DialogContent></Dialog>; }
